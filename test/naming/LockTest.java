...
public class LockTest extends NamingTest
{
    ...
    private void testBadPaths() throws TestFailed
    {
        ...
        try
        {
            LockRequest request = new LockRequest(non_existent_path.toString(), false);
            HttpResponse<String> response = getResponse("/lock", SERVICE_PORT, request);
            String exception_type = gson.fromJson(response.body(), ExceptionReturn.class).exception_type;
    ...
    protected void initialize() throws TestFailed
    {
        ...
        try
        {
            ...
            storage_server.start(REGISTRATION_PORT, new Path[] {file1, file2},
                    null);
        }
        catch(Throwable t)
        {
    ...
    private abstract class LockUser implements Runnable
    {
        ...
        public void run()
        {
            ...
            try
            {
                LockRequest request = new LockRequest(path.toString(), exclusive);
                HttpResponse<String> response = getResponse("/lock", SERVICE_PORT, request);
